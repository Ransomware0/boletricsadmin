...
type DropdownProps = {
  ...
  contentClass?: string;
  img?: ReactNode;
  onSelectOption: (option: keyof Transaction) => void;
};
...
const DropdownDisbursements = ({
  ...
  const handleItemClick = (item: string) => {
    setSelected(item); // Set selected item in parent component
    onSelectOption(item as keyof Transaction); // Pass selected item to parent component as keyof Event
      ...
      >
        {items?.map((item) => (
          <li
            ...
            key={item}
            className={`px-4 py-2 cursor-pointer text-sm rounded-md duration-300 ${selected == item && "bg-primary/5 "
              }`}
          >
            {item}
          </li>
...
export default DropdownDisbursements;
